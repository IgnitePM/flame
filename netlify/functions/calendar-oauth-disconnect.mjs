...
/**
 * Any staff: disconnect Google Calendar for the signed-in caller.
 * POST → { ok: true }
 */
export default async (req) => {
  ...
  try {
    ...
    console.error('[calendar-oauth-disconnect]', err);
    return new Response(
      JSON.stringify({ error: err?.message || 'Could not disconnect Calendar.' }),
