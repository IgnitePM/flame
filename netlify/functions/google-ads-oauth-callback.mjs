...
/**
 * Google OAuth redirect target for company Google Ads.
 * GET ?code=&state=  (or ?error=)
 */
export default async (req) => {
  const url = new URL(req.url);
  const base = appBaseUrl();
  const configUrl = `${base}/admin?ads=`;

  const fail = (reason) =>
    Response.redirect(`${configUrl}error&adsMsg=${encodeURIComponent(reason)}`, 302);
  ...
  try {
    const { uid, email } = verifyAdsOAuthState(state);
    const tokens = await exchangeCodeForTokens(code);
    if (!tokens.access_token) throw new Error('No access token returned.');

    const adsEmail = (await fetchGoogleUserEmail(tokens.access_token)) || email;
    const now = Date.now();
    ...
    return fail(err?.message || 'Google Ads connect failed.');
  }
};
