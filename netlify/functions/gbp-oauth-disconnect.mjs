...
/**
 * Admin/billing: disconnect company Google Business Profile.
 * POST → { ok: true }
 */
export default async (req) => {
  ...
  try {
    ...
    console.error('[gbp-oauth-disconnect]', err);
    return new Response(
      JSON.stringify({ error: err?.message || 'Could not disconnect GBP.' }),
