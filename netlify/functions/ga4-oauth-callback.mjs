...
/**
 * Google OAuth redirect target for company GA4.
 * GET ?code=&state=  (or ?error=)
 */
export default async (req) => {
  const url = new URL(req.url);
  const base = appBaseUrl();
  const configUrl = `${base}/admin?ga4=`;

  const fail = (reason) =>
    Response.redirect(`${configUrl}error&ga4Msg=${encodeURIComponent(reason)}`, 302);
  ...
  try {
    const { uid, email } = verifyGa4OAuthState(state);
    const tokens = await exchangeCodeForTokens(code);
    if (!tokens.access_token) throw new Error('No access token returned.');

    const ga4Email = (await fetchGoogleUserEmail(tokens.access_token)) || email;
    const now = Date.now();
    ...
    return fail(err?.message || 'GA4 connect failed.');
  }
};
