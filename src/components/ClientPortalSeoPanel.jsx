...
export default function ClientPortalSeoPanel({ client, dateFromMs, dateToMs }) {
  ...
  const [gbpReport, setGbpReport] = useState(null);
  ...
  const loadGbp = async ({ forceRefresh = false } = {}) => {
    if (!client?.id) return;
    setGbpLoading(true);
    setGbpError('');
    try {
      const resp = await authedFetch('/.netlify/functions/portal-gbp', {
        clientId: client.id,
        dateFrom: range.from,
        dateTo: range.to,
        ...(forceRefresh ? { forceRefresh: true } : {}),
      ...
      setGbpReport(null);
      setGbpError(err?.message || 'Could not load Local SEO report.');
    } finally {
      setGbpLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              ...
              {gbpReport?.cached
                ? ` · Cached${
                    gbpReport.cacheAgeSec
                      ? ` ${Math.max(1, Math.round(gbpReport.cacheAgeSec / 60))}m ago`
                      : ''
                  }`
                : ''}
            </p>
          ...
          <button
            type="button"
            onClick={() => loadGbp({ forceRefresh: true })}
        ...
        {gbpLoading ? (
          <div className="bg-white border border-slate-200 rounded-[32px] p-12 shadow-sm flex flex-col items-center justify-center gap-3 text-slate-500">
            ...
            <p className="text-sm font-bold">Loading Local SEO…</p>
          </div>
        ) : gbpError ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              <StatCard label="Search views" value={formatNum(totals.searchViews)} />
              <StatCard label="Maps views" value={formatNum(totals.mapViews)} />
              <StatCard label="Listing views" value={formatNum(totals.listingViews)} />
              <StatCard label="Website clicks" value={formatNum(totals.websiteClicks)} />
              <StatCard label="Calls" value={formatNum(totals.calls)} />
              <StatCard
